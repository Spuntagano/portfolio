...
export default function App() {
  ...
  const [questionIndex, setQuestionIndex] = React.useState(new Array(questions.length).fill(0));
  ...
  function addMessage(question, awnser, index) {
  ...
  return <>
    ...
    <div className={`input-container ${isDown ? 'down': ''}`}>
      {!isDown && <h2>{language === 'fr' ? 'Comment puis-je vous aider?' : 'How can I help you today?'}</h2>}
      <div className="mid">
        <div className="input">
          ...
          <div className="input-box">
            {isLoading ? <div className="loader"></div> :
              <>
                {questionIndex.map((i, index) => {
                  if (!questions[index][i]) {
                    return null;
                  }

                  return <button onClick={() => addMessage(questions[index][i].content[language], questions[index][i].awnser[language], index)} key={index}>
                    {questions[index][i].content[language]}
                  </button>
                })}
                </>}
          </div>
